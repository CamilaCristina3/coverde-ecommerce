...
class IndexView(TemplateView):
    ...
    def get_cover_image(self):
        ...
        covers_folder = os.path.join(settings.BASE_DIR, 'static', 'css', 'images', 'covers')
        try:
            image_files = [
                f for f in os.listdir(covers_folder) 
                if f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif'))
            ]
            selected_image = random.choice(image_files) if image_files else 'farm-background.jpg'
            return f'images/covers/{selected_image}'
        except (FileNotFoundError, OSError):
            return 'images/covers/farm-background.jpg'
