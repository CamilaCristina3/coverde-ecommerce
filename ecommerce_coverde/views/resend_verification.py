...
class ResendVerificationView(FormView):
    template_name = 'ecommerce_coverde/auth/resend_verification.html'
    form_class = ResendVerificationForm
    success_url = reverse_lazy('ecommerce_coverde:login')
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            verification_url = self.request.build_absolute_uri(
                reverse('ecommerce_coverde:verificar_email') + f'?token={new_token}&uid={user.id}'
            )
